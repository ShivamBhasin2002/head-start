...
import { ProductPricing } from "@/lib/api";
...
export function POICard({
  ...
  const { tgid, pricing } = poi;

  // Use pricing from API if available, otherwise fall back to hardcoded values
  const discountPercentage = pricing?.listingPrice?.bestDiscount;
  const price = pricing?.listingPrice
    ? `${pricing.listingPrice.currencyCode} ${pricing.listingPrice.finalPrice.toFixed(2)}`
    : undefined;
  const originalPrice = pricing?.listingPrice?.originalPrice;
        ...
        {tgid && (
          ...
          >
            <div className="flex flex-col gap-0.5">
              <span
                className={`font-[halyard-text] font-medium text-[12px] gap-2 leading-[16px] tracking-[0] ${isLight ? "text-[#666666]" : "text-[#ffffff50]"}`}
              >
                from{" "}
                {originalPrice &&
                  originalPrice > pricing?.listingPrice?.finalPrice && (
                    <span
                      className={`font-[halyard-text] font-medium text-[12px] leading-[16px] tracking-[0] line-through ${
                        isLight ? "text-[#666666]" : "text-[#ffffff50]"
                      }`}
                    >
                      {pricing?.listingPrice?.currencyCode}{" "}
            ...
            <a href={`https://www.headout.com/tour/${tgid}`} target="_blank">
              <svg
                width="146"
                height="36"
                viewBox="0 0 146 36"
